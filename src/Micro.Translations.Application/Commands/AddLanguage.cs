...
public static class AddLanguage
{
    ...
    public class Handler(ILanguageRepository languages, IExecutionContext context) : IRequestHandler<Command>
    {
        public async Task Handle(Command command, CancellationToken token)
        {
            ...
            if (await languages.GetAsync(languageId, token) != null) throw new AlreadyExistsException(nameof(Language), languageId);

            if (await languages.GetAsync(projectId, code, token) != null) throw new AlreadyInUseException(nameof(Language), code);
